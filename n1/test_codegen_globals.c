...
// Function Prototypes
void mph_test_global_bool_var(MorphContext* ctx, void* _env_void);
...
mph_codegen_Codegen* mph_codegen_new_codegen(MorphContext* ctx, void* _env_void) {
	return ({ mph_codegen_Codegen* _t = (mph_codegen_Codegen*)mph_alloc(ctx, sizeof(mph_codegen_Codegen), &mph_ti_mph_codegen_Codegen); mph_gc_push_root(ctx, (void**)&_t); _t->pass_log = mph_string_new(ctx, ""); _t->indent_level = 0; _t->current_prefix = mph_string_new(ctx, "mph_"); _t->output = mph_string_new(ctx, ""); _t->type_defs = mph_string_new(ctx, ""); _t->func_defs = mph_string_new(ctx, ""); _t->temp_index = 0; _t->has_main = 0; _t->global_defs = mph_string_new(ctx, ""); _t->prototypes = mph_string_new(ctx, ""); _t->entry_body = mph_string_new(ctx, ""); mph_gc_pop_roots(ctx, 1); _t; });
}
...
mph_ast_Program* mph_ast_MakeProgram(MorphContext* ctx, void* _env_void) {
	...
	mph_gc_push_root(ctx, (void**)&stmt);
	return ({ mph_ast_Program* _t = (mph_ast_Program*)mph_alloc(ctx, sizeof(mph_ast_Program), &mph_ti_mph_ast_Program); mph_gc_push_root(ctx, (void**)&_t); _t->base = base; _t->statements_count = 0; _t->var_statements_count = 0; _t->var_statement = stmt; _t->has_var_statement = 0; mph_gc_pop_roots(ctx, 1); _t; });
	mph_gc_pop_roots(ctx, 2);
}
...
mph_ast_WhileStatement* mph_ast_make_while_statement(MorphContext* ctx, void* _env_void, mph_int line, mph_int column) {
	...
	mph_gc_push_root(ctx, (void**)&base);
	return ({ mph_ast_WhileStatement* _t = (mph_ast_WhileStatement*)mph_alloc(ctx, sizeof(mph_ast_WhileStatement), &mph_ti_mph_ast_WhileStatement); mph_gc_push_root(ctx, (void**)&_t); _t->has_body = 0; _t->base = base; mph_gc_pop_roots(ctx, 1); _t; });
	mph_gc_pop_roots(ctx, 1);
}
...
void mph_test_global_bool_var(MorphContext* ctx, void* _env_void) {
	mph_native_print(ctx, mph_string_new(ctx, "[Test] Global Bool Var Compilation\n"));
	mph_ast_Program* program = mph_ast_MakeProgram(ctx, NULL);
	mph_gc_push_root(ctx, (void**)&program);
	mph_ast_VarStatement* stmt = ({ MorphString* _arg_1333 = mph_string_new(ctx, "flag"); mph_gc_push_root(ctx, (void**)&_arg_1333); MorphString* _arg_1334 = mph_string_new(ctx, "bool"); mph_gc_push_root(ctx, (void**)&_arg_1334); mph_ast_VarStatement* _ret_1335 = mph_ast_MakeVarStatement(ctx, NULL, _arg_1333, _arg_1334, 1, 1); mph_gc_pop_roots(ctx, 2); _ret_1335; });
	mph_gc_push_root(ctx, (void**)&stmt);
	({ mph_ast_VarStatement* _obj_1336 = stmt; mph_gc_push_root(ctx, (void**)&_obj_1336); _obj_1336->has_value = 1; mph_gc_pop_roots(ctx, 1); });
	({ mph_ast_VarStatement* _obj_1337 = stmt; mph_gc_push_root(ctx, (void**)&_obj_1337); MorphString* _val_1338 = mph_string_new(ctx, "benar"); mph_gc_push_root(ctx, (void**)&_val_1338); _obj_1337->value_literal = _val_1338; mph_gc_pop_roots(ctx, 2); });
	({ mph_ast_VarStatement* _obj_1339 = stmt; mph_gc_push_root(ctx, (void**)&_obj_1339); _obj_1339->value_token_type = mph_token_TOKEN_BENAR; mph_gc_pop_roots(ctx, 1); });
	({ mph_ast_Program* _obj_1340 = program; mph_gc_push_root(ctx, (void**)&_obj_1340); _obj_1340->statements_count = 1; mph_gc_pop_roots(ctx, 1); });
	({ mph_ast_Program* _obj_1341 = program; mph_gc_push_root(ctx, (void**)&_obj_1341); _obj_1341->var_statements_count = 1; mph_gc_pop_roots(ctx, 1); });
	({ mph_ast_Program* _obj_1342 = program; mph_gc_push_root(ctx, (void**)&_obj_1342); mph_ast_VarStatement* _val_1343 = stmt; mph_gc_push_root(ctx, (void**)&_val_1343); _obj_1342->var_statement = _val_1343; mph_gc_pop_roots(ctx, 2); });
	({ mph_ast_Program* _obj_1344 = program; mph_gc_push_root(ctx, (void**)&_obj_1344); _obj_1344->has_var_statement = 1; mph_gc_pop_roots(ctx, 1); });
	mph_codegen_Codegen* cg = mph_codegen_NewCodegen(ctx, NULL);
	mph_gc_push_root(ctx, (void**)&cg);
	MorphString* output = ({ mph_codegen_Codegen* _arg_1345 = cg; mph_gc_push_root(ctx, (void**)&_arg_1345); mph_ast_Program* _arg_1346 = program; mph_gc_push_root(ctx, (void**)&_arg_1346); MorphString* _ret_1347 = mph_codegen_Compile(ctx, NULL, _arg_1345, _arg_1346); mph_gc_pop_roots(ctx, 2); _ret_1347; });
	mph_gc_push_root(ctx, (void**)&output);
	mph_native_print(ctx, mph_string_new(ctx, "  Output:\n"));
	mph_native_print(ctx, output);
	mph_native_print(ctx, mph_string_new(ctx, "\n"));
	mph_native_print(ctx, mph_string_new(ctx, "  â Global bool var test completed\n\n"));
	mph_gc_pop_roots(ctx, 4);
}

mph_int mph_main(MorphContext* ctx, void* _env_void) {
	mph_test_global_bool_var(ctx, NULL);
	return 0;
}
